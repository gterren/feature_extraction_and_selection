...
def _wind_velocity_field_svm(_tools, XY_tr_, UV_tr_, w_tr_, XY_ts_, UV_ts_, w_ts_, dXYZ_, XY_stdz_, xy_stdz_, _stdz_x, _stdz_y,
                             N_y, N_x, step_size, svm, kernel, degree, weights, CV, N_grid, N_kfold, N_BO_iterations, display):
    # How much error in the Regression?
    def __regression_error_analysis(_tools, UV_tr_, UV_ts_, UV_tr_hat_, UV_ts_hat_, w_tr_, w_ts_, verbose = True):
        ...
        def __E(y_, y_hat_):
            return y_ - y_hat_
        def __WRMSE(y_, y_hat_, w_):
            return np.sqrt(np.average((y_ - y_hat_)**2, weights = w_))
        path = r'/users/terren/cloud_feature_extraction/software/cloud_feature_extraction/images/'
        r_0_ = __E(UV_ts_[..., 0], UV_ts_hat_[..., 0])
        r_1_ = __E(UV_ts_[..., 1], UV_ts_hat_[..., 1])
        m_1_ = __E(np.sqrt(UV_ts_[..., 0]**2 + UV_ts_[..., 1]**2), np.sqrt(UV_ts_hat_[..., 0]**2 + UV_ts_hat_[..., 1]**2))
        ...
        plt.hist(m_1_)
        plt.grid()
        plt.savefig(path + '{}.png'.format(int(_tools.t_[_tools.j])))
        ...
        return __WRMSE(np.sqrt(UV_ts_[:, 0]**2 + UV_ts_[:, 1]**2), np.sqrt(UV_ts_hat_[:, 0]**2 + UV_ts_hat_[:, 1]**2), w_ts_), __WRMSE(UV_ts_[:, 0], UV_ts_hat_[:, 0], w_ts_), __WRMSE(UV_ts_[:, 1], UV_ts_hat_[:, 1], w_ts_),
    ...
    #WRMSE_tr, WRMSE_ts, MRSE_tr, MRSE_ts, WMAE_tr, WMAE_ts, MAE_tr, MAE_ts = __regression_error_analysis(_tools, UV_tr_, UV_ts_, uv_tr_hat_, uv_ts_hat_, w_tr_, w_ts_, verbose = True)
    WRMSE_ts, X_WMAE_ts_, Y_WMAE_ts_ = __regression_error_analysis(_tools, UV_tr_, UV_ts_, uv_tr_hat_, uv_ts_hat_, w_tr_, w_ts_, verbose = True)
    # Flow Daynamic Constrain Operators Definition
    dV_, dD_ = _flow_dynamics_constrains_operator(dXYZ_, N_y, N_x)
    ...
    return UV_hat_, [params_, WRMSE_ts, X_WMAE_ts_, Y_WMAE_ts_, D, V]
...
__all__ = ['_wind_velocity_field_coordinates', '_wind_velocity_field_svm']
