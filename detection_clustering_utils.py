...
def _EM_GMM(X_, n_clusters = 2, n_init = 3, tol = 0.1, metric = 1,
            reg_term = 1e-5, n_iter = 1000, verbose = False, epsilon = 0.):
    init_lim_low = X_.min()
    init_lim_up  = X_.max()
...
def _EM_VmMM(X_, n_clusters = 2, n_init = 3, tol = 0.1, metric = 1, n_iter = 1000,
             reg_term = 1e-5, w_init_ = None, verbose = False, epsilon = 0.):
    init_lim_low = 1e-5
    init_lim_up  = 250.
    # Check for EM algoritm convergenceG
    def __convergence(ll_k_1_, ll_k, tol):
...
def _EM_GaMM(X_, n_clusters = 2, n_init = 3, tol = 0.1, n_iter = 1000, metric = 1,
             reg_term = 1e-5, w_init_ = None, verbose = False, epsilon = 0):
    init_lim_low = .001
    init_lim_up  = 250.
    # Check for EM algoritm convergence
    ...
    def __rand_init(X_):
        ...
        for k in range(n_clusters):
            # Define Variable Boundaries for Optimization
            bounds_.insert(0, (.01, init_lim_up))
            bounds_.append((.01, init_lim_up))
